...
public class TypeScriptSchemaOptions
{
	...
	public string OperationsFolder { get; set; } = "operations";
...
public record MissingTypeScriptSchemaOptionsDiagnostic(Uri Uri, Location Location) : DiagnosticBase(Location)
{
	public override IReadOnlyList<string> GetTextArguments() => [Uri.OriginalString];
	public static DiagnosticException.ToDiagnostic Builder(Uri uri) => (Location) => new MissingTypeScriptSchemaOptionsDiagnostic(uri, Location);
}
